...
7. **Commerce Service** — Stripe integration, Redis cart, checkout, orders, subscriptions, webhooks (35 files + 5 infra updates)

### Phase 4: Mobile Integration + Polish
8. **Mobile app SDK/hooks** — React Native hooks to call services via gateway
9. **Cross-service integration tests**
10. **End-to-end testing** — Full flows from mobile through gateway to services
...
## 14. Implementation Notes (Phase 3 — Completed)

Phase 3 added the Commerce Service (35 files) plus 5 infrastructure file updates. Key lessons:

### 14.1 Dual Database Pattern (MongoDB + Redis)
Commerce is the first service using both MongoDB (durable: orders, profiles, subscriptions) and Redis (ephemeral: carts with 7-day TTL). The FastAPI lifespan connects to both and stores them on `app.state`. Redis client uses `redis.asyncio.from_url()` with `decode_responses=True`.

### 14.2 Gateway Proxy Body Forwarding (fixRequestBody)
`express.json()` middleware consumes the request body stream before `http-proxy-middleware` can forward it. This caused POST requests through the gateway to arrive with empty bodies at backend services (manifesting as `starlette.requests.ClientDisconnect` errors and 30-second timeouts). Fixed by importing and calling `fixRequestBody(proxyReq, req, res)` from `http-proxy-middleware` in the `proxyReq` handler. This was applied globally to all proxy routes, not just commerce.

### 14.3 Webhook Auth Bypass via next('route')
Stripe webhook calls don't carry JWT auth — they rely on signature verification instead. The gateway handles this with Express's `next('route')` pattern: the authenticated `/commerce` route checks if `req.path.startsWith('/webhooks')` and if so, calls `next('route')` to skip auth, falling through to an identical but unauthenticated proxy route below it. Route ordering is critical — the auth route must come before the fallback.

### 14.4 Stripe Webhook Raw Body
The FastAPI webhook route reads the raw body with `await request.body()` and passes it to `stripe.Webhook.construct_event()` for signature verification. The route must NOT declare a Pydantic body model — that would parse the JSON and lose the raw bytes, breaking signature verification.

### 14.5 Server-Side Pricing
Cart totals are always recomputed server-side by `_recalculate_totals()`. Client-provided totals are never trusted. Tax and shipping are set to 0 for now (digital goods).

### 14.6 Stripe SDK Sync Calls in FastAPI
`stripe-python` uses synchronous HTTP calls. In FastAPI (async), these work fine because they're quick I/O operations. If blocking becomes a performance issue under load, wrap in `asyncio.to_thread()`.

### 14.7 Permissions Sync on Subscription Events
When subscriptions are created, updated, or deleted, the Commerce Service calls the Permissions Service (`POST /subscriptions/{user_id}`) via httpx with the internal API key. This keeps subscription tier and permissions in sync without manual intervention.

### 14.8 Commerce Gateway pathPrefix
Like the LLM Service, Commerce has routes at root level (`/cart/*`, `/checkout/*`, `/orders/*`, `/webhooks/*`) with no common prefix. The gateway uses `pathPrefix: ''`.

### 14.9 FakeRedis for Testing
Tests use `fakeredis[json]` (`fakeredis.aioredis.FakeRedis`) as an in-process async Redis mock. Combined with `mongomock-motor` for MongoDB, this gives full dual-database test coverage without running actual database containers.

### 14.10 Order ID Format
Orders use a human-readable ID format: `ORD-YYYYMMDDHHMMSS-XXXXXX` (timestamp + 6-char random hex). Generated by `generate_order_id()` in `models/orders.py`.
